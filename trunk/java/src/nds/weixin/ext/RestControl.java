...
public class RestControl {
	...
	private void openCard(JSONObject restjo) throws Exception{
		if(restjo==null||!restjo.has("vipid")||!restjo.has("ad_client_id")){
			logger.debug("params->"+restjo.toString());
			logger.debug("params must not null");
			return;
		}
		List all=null;
		...
		if(all!=null&&all.size()>0) {
			...
			throw new Exception("not find WX_INTERFACESET ad_client_id->"+ad_client_id);
		}
		...
		try{
			if(sp.getCode().equals("0")){
				...
				List al = QueryEngine.getInstance().doQueryList("select vp.wechatno,vp.vipcardno,vp.store_id,NVL(vt.ISSEND,'N'),cp.num,cp.usetype1,vt.code,nvl(cp.value,'0'),nvl(vp.integral,0),to_char(decode(nvl(cp.validay,0),0,nvl(cp.starttime,sysdate), sysdate), 'YYYYMMDD'),to_char(decode(nvl(cp.validay,0),0, nvl(cp.endtime, add_months(sysdate, 1)),sysdate+cp.validay), 'YYYYMMDD')"+
						...
						" WHERE vp.id=?",new Object[] {vipid},conn);
				...
				if(al!=null&&al.size()>0) {
					...
					throw new Exception("not find vipid->"+vipid);
				}
			...
			if(jo.optInt("errCode",-1)==0) {
				if(!("N".equalsIgnoreCase(isSendCoupon)||"1".equalsIgnoreCase(couponUseType))) {
				...
				if(insertcount>0&&jo.has("result")&&jo.optJSONObject("result").has("card")&&jo.optJSONObject("result").optJSONObject("card").has("no")) {
					String updatevip="UPDATE wx_vip SET docno=? WHERE ID=?";
					insertcount=QueryEngine.getInstance().executeUpdate(updatevip,new Object[] {jo.optJSONObject("result").optJSONObject("card").optString("no"),vipid},conn);
					if(insertcount<=0) {
						logger.debug("open offline code update WX_VIP error count->"+insertcount);
						throw new Exception("open offline code update WX_VIP error count->"+insertcount);
					}else {
						try {conn.commit();}
						catch(Exception e) {
							logger.debug("open offline code commit error->"+e.getLocalizedMessage());
							throw new Exception("open offline code commit error->"+e.getLocalizedMessage());
						}
					}
				}else {
					if(insertcount<=0) {
						...
						throw new Exception("open offline code result not find no[线下卡号] error->"+jo.optString("errMessage"));
					}
				}
				
			}else {
				logger.debug("open offline code error->"+jo.optString("errMessage"));
				throw new Exception("open offline code error->"+jo.optString("errMessage"));
			}
	...
	private void bindCard(JSONObject restjo) throws Exception{
		...
		if(sp.getCode().equals("0")){
			...
			if(al!=null&&al.size()>0) {
				...
				throw new Exception("bind offline code error not find vipid->"+vipid);
				//return isSuccessfull;
			}
			params.put("format","JSON");
			params.put("client","");
		...
		if(recode!=0) {
			throw new Exception("bind offline code error->"+jo.optString("errMessage"));
		}else {
			String sql="";
			int viptypeid=0;
			...
			sql="select vb.id from wx_vipbaseset vb WHERE vb.code=? AND vb.ad_client_id=?";
			   try {
			   		...
			   		if(viptypeid<=0) {
						throw new Exception("bind offline code error not find viptype->"+cardjo.optString("level"));
			   		}
			   }catch(Exception e) {
				   throw new Exception("bind offline code error not find viptype->"+cardjo.optString("level"));
			   }
			//sql="UPDATE wx_vip v SET integral=?,lastamt=?,viptype=? WHERE v.id=?";
			  sql="UPDATE WX_VIP V SET V.INTEGRAL=?,V.LASTAMT=?,V.VIPTYPE=?,V.DOCNO=?,V.RELNAME=?,V.EMAIL=?,V.PHONENUM=?,V.STORE_ID=?,V.CONTACTADDRESS=?,V.BIRTHDAY=?,V.GENDER=?"+
				  " WHERE V.ID=?";
			int count=0;
			count=QueryEngine.getInstance().executeUpdate(sql,new Object[] {cardjo.optInt("credit"),cardjo.optDouble("balance"),viptypeid,cardjo.optString("no"),cardjo.optString("name",""),cardjo.optString("email",""),cardjo.optString("phonenum",""),cardjo.optInt("store_id",0),cardjo.optString("address",""),cardjo.optString("birthday"),cardjo.optString("gender"),vipid});
			if(count<=0) {
				logger.debug("bind offline code update vip error->"+count);
				throw new Exception("bind offline code update vip error->"+count);
			}
			sql="UPDATE WX_VIPINFO V SET V.isopencard=2,V.VIPTYPE_ID=?,v.relname=?,v.email=?,v.phonenum=?,v.otheraddress=?,v.birthday=?,v.gender=?,v.province=?,v.city=?,v.regionid=?"+
				  " WHERE v.wx_vip_id=?";
			count=QueryEngine.getInstance().executeUpdate(sql,new Object[] {viptypeid,cardjo.optString("name",""),cardjo.optString("email",""),cardjo.optString("phonenum",""),cardjo.optString("address",""),cardjo.optString("birthday"),cardjo.optString("gender"),cardjo.optString("province",""),cardjo.optString("city",""),cardjo.optString("depart",""),vipid});
			if(count<=0) {
				logger.debug("bind offline code update vipinfo error->"+count);
				throw new Exception("bind offline code update vipinfo error->"+count);
			}else {
	...
	private void updateCard(JSONObject restjo) throws Exception{
		...
		if(sp.getCode().equals("0")){
			...
			System.out.println("validateSing error->"+sp.toString());
			throw new Exception("validateSing error->"+sp.toString());
			//return isSuccessfull;
		}
		ValueHolder vh=null;
		try{
			...
			throw new Exception("微生活网络通信障碍! "+tx.getMessage());
			//return;
		}
		String result=(String) vh.get("message");
		...
		int recode=jo.optInt("errCode");
		if(recode!=0) {
			throw new Exception("update offline error->"+jo.optString("errMessage"));
		}
	}
	
	/**
	前提
	双方约定通信加密使用的通信密匙即 skey。 生成规则
	将 cardid, ts, skey 拼接起来的字符串进行 md5 加密即得到 sig 拼接顺序必须为 cardid, ts, skey
	md5 后的结果为小写字符
	cardid 存放与 we_vip_cost
	* @param request
	* @return
	* @throws Exception 
	*/
