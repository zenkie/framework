/******************************************************************
*
*$RCSfile: DbConnectionDefaultPool.java,v $ $Revision: 1.1.1.1 $ $Author: Administrator $ $Date: 2005/03/15 11:23:23 $
*
*$Log: DbConnectionDefaultPool.java,v $
*Revision 1.1.1.1  2005/03/15 11:23:23  Administrator
*init
*
*Revision 1.6  2004/02/02 10:42:38  yfzhu
*<No Comment Entered>
*
*Revision 1.5  2003/08/17 14:25:12  yfzhu
*before adv security
*
*Revision 1.4  2003/05/29 19:40:16  yfzhu
*<No Comment Entered>
*
...
********************************************************************/
/**
*	Modified to wrap connection for backup log
*   @see farm.backup.client.ConnectionAgent
*/
...
public class DbConnectionDefaultPool  {
        String driver = "oracle.jdbc.driver.OracleDriver";
        String server = "jdbc:oracle:thin:@mit:1521:hpl1000";
        String username = "nds2";
        String password = "abc123";
        String minConnections = "1";
        String maxConnections = "15";
        String logPath = "/aic/dbpool.log";
        String connectionTimeout = "0.0003472 "; // 30 seconds
/*
        String driver = "oracle.jdbc.driver.OracleDriver";
        String server = "jdbc:oracle:thin:@localhost:1521:test";
        String username = "nds5";
        String password = "abc123";
        String minConnections = "1";
        String maxConnections = "15";
        String logPath = "/aic/dbpool.log";
...
/**/
    ...
    private static DbConnectionDefaultPool instance=null;
    public static synchronized DbConnectionDefaultPool getInstance(){
        if(instance==null) instance=new DbConnectionDefaultPool();
        return instance;
    }
    /**
    ...
    public Connection getConnection() {
        ...
        Connection cw=new ConnectionWrapper(connectionPool.getConnection(), connectionPool);
        Connection fc=null;
        if( connectionClass !=null){
        	// wrapper twice
        	try{
        		java.lang.reflect.Constructor co= connectionClass.getConstructor(new Class[]{Connection.class});
        		fc= (Connection)co.newInstance(new Object[]{cw});
        	}catch(Exception e){
        		System.err.println("Could not contruct class:"+connectionClass+", contructor xxx(Connection)"+
        		" not found.");
        		fc=cw;
        	}
    	}else{
    ...
    private class ConnectionPool implements Runnable {
        ...
        /*----------added by naeco to test where connection not released when all connections are used--*/
        private String[] callerStacks;
        /*---------------end by naeco -------*/
                    ...
                    String logFileString, double maxConnTime) throws IOException
        {
            ...
            /*---naeco begin------*/
            callerStacks=new String[maxConns];
            /*------------end ------*/
        ...
        public Connection getConnection() {
            ...
            if(available){
                ...
                for(int outerloop=1; outerloop<=10; outerloop++) {
                        ...
                        do {
                            synchronized(connStatus) {
                                                    ...
                                                    (! connPool[roundRobin].isClosed()))
                                {
                                    ...
                                    callerStacks[roundRobin]= baos.toString();
                                    /*--------end naeco---*/
                                    connLast = roundRobin;
                                    gotOne = true;
                                    break;
                    ...
                    if(gotOne) {
                        ...
                        log.println("-----> Connections Exhausted("+currConnections+")!  Will wait and try " +
                            "again in loop " +
            ...
            /*-- naeco ---*/
            if(conn ==null){
                // here's exhausted, print it out
            long elaps= System.currentTimeMillis() - 20* 1000;// 20 seconds

            for(int i=0;i< connPool.length;i++){
                if( connStatus[i]==1 && connLockTime[i] < elaps){
                        log.println("------dead connection["+ i+"]( elaps="+(System.currentTimeMillis()-connLockTime[i])/1000+" seconds,locktime="+ (new Date(connLockTime[i]))+"):");
                        log.println( callerStacks[i]);
                        // since it's dead, free it to the unrequested one if over 2 hours
                        if((System.currentTimeMillis()-connLockTime[i])/1000 >7200)
        ...
        public String freeConnection(Connection conn) {
            ...
            if(thisconn >= 0) {
                ...
                //log.println("Freed connection " + String.valueOf(thisconn) + " normal exit: ");

                /*-----naeco-------*/
                callerStacks[thisconn]=null;
                /*-----end naeco-----*/
            }
            else {
...
/*                Properties info=new Properties();
                info.setProperty("user",dbLogin);
                info.setProperty("password",dbPassword);
                info.setProperty("charSet","GB_2312");
                String[] s= interbase.interclient.CharacterEncodings.getSupportedEncodings();
                for(int k=0;k<s.length;k++)
                  System.out.println(s[k]);

                System.out.println( "final gb:"+ interbase.interclient.CharacterEncodings.getInterBaseCharacterSetName("GBK"));
              connPool[i]= DriverManager.getConnection(dbServer,info);
