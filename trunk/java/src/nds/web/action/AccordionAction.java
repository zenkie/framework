...
public class AccordionAction extends WebActionImpl {
	...
	public String toHTML(Locale locale, Map env) {
		/**
		 * <div> <h3><a>head name</a></h3> <div
		 * style="height:300px;max-height:269px"> max 12 div
		 * tabimg="<img src=\""+StringUtils.escapeForXML(table.getAccordico())+
		 * "\" style=\"height:16px;width:20px;\"></img>"; <div
		 * class="accordion_headings"
		 * onclick="javascript:pc.navigate('15000')">tabimg<a>采购核价单</a></div>
		 * </div> </div> one action simaple
		 */
		...
		if (ate.equals(WebAction.ActionTypeEnum.URL)
			...
			String ptype = (String) env.get("ptype");// 根据类型判断是否是oto下的动作定义
			if (userWeb == null) {
				throw new NDSRuntimeException("Must have userWeb set in env");
			}
			try {
				sb.append(writeXmlFromJSONArray(
						new JSONArray(this.getScript()), locale, userWeb, ptype));
	...
	private StringBuffer writeXmlFromJSONArray(JSONArray jsonArray,
			Locale locale, UserWebImpl userweb, String ptype) throws Exception {
		...
		for (int i = 0; i < jsonArray.length(); i++) {
			...
			stringbuffer.append(ptype == null ? writeXmlFromJSON(jsonobject,
					locale, userweb) : writeXmlFromJSONOTO(jsonobject, locale,
					userweb));
		}
		return stringbuffer;
	}

	// 如果ptype类型为oto则调用该方法生成文档格式
	private Object writeXmlFromJSONOTO(JSONObject jo, Locale local,
		...
		Object text_class = jo.optString("class");// 折叠菜单样式
		...
		if ((Validator.isNotNull(ico_png = jo.optString("icon")))
			...
			ico_png = "<img src=\"/html/nds/images/" + ico_png
					+ "\" style=\"height:16px;width:20px;\"></img>";
		}
		...
		if ((folder = jo.optJSONArray("folder")) != null) {
			...
			if (Validator.isNull((String) text_lable))
				text_lable = "自定义菜单";
			if (Validator.isNull((String) text_class))
				text_class = "fa-system fa-color-purple";
			// System.out.println(((JSONArray) folder).length());
			int folder_size = ((JSONArray) folder).length();
			folder = writeXmlFromJSONArray((JSONArray) folder, local, userweb,
					"oto").toString();
			if (folder != null && (folder.toString().length() != 0)
					&& (folder.toString().trim().length() != 0)) {
				...
				if (folder_size + Accordnum >= 12) {
					...
					accord_xml
							...
							.append(text_class)
							.append("\"><b/><b/></i>")
							.append("<span class=\"pull-right fa-angle-down\"/><span>")
							.append(text_lable)
							.append("</span></a><ul class=\"nav lt\" style=\"max-height:500px\">")
							.append((String) folder).append("</ul></li>");
		...
		// 判断table 标签
		{
			...
			if (table != null) {
				...
				try {
					...
					accord_xml
							.append("<li><a href=\"javascript:void(0);\" onclick=\"javascript:")
							.append(action).append("\">")
							.append("<i class=\"fa fa-angle-right\"><b/></i>")
							.append("<span>").append((String) text_lable)
							.append("</span></a></li>");
	...
	private Object writeXmlFromJSON(JSONObject jo, Locale local,
		...
		// 判断table 标签
		{
			...
			if (table != null) {
				...
				try {
					...
					// onclick="javascript:pc.navigate('15000')">tabimg<a>采购核价单</a></div>
					accord_xml
							.append("<div class=\"accordion_headings")
							// .append(ico_png)
							.append("\" onclick=\"javascript:").append(action)
							.append("\">").append(ico_png).append("<a>")
							.append((String) text_lable).append("</a></div>");
				} catch (NDSSecurityException e) {
				}
			}
		} else if (Validator.isNotNull(jo.optString("rpt")))
		// 判断报表标签RPT
		{
			...
			accord_xml
					...
					.append(jo.optString("script")).append("\">")
					.append(ico_png).append("<a>").append((String) text_lable)
					.append("</a></div>");
			/*
			 * accord_xml.append("<tree icon=\"").append(ico_png)
			 * .append("\" text=\"").append((String) text_lable)
