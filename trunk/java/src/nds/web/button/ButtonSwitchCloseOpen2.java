 ...
 * 显示的按钮构造规则 TABLENAME+"-swtich-open"/"-switch-close"
 * 
 */
public class ButtonSwitchCloseOpen2 extends ButtonCommandUI_Impl{
	...
	protected String getCaption(HttpServletRequest request, Column column, int objectId ){
		...
		return  nds.util.MessagesHolder.getInstance().getMessage(locale, 
				column.getTable().getName().toLowerCase()+"-"+( isStop(column,objectId)? "switch-open":"switch-close"));
	}	
}
