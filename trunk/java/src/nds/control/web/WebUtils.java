...
public final class WebUtils {
    ...
    private static int systemMode=0; // 1 for debug, 2 for production, 0 will load from propery file
	
    private static ServletContext context=null;
    
    /**
     * For accelerating locating adclient from domain name, make cache here
     * Key: web_client.DOMAIN(String), value: [ad_client.id (Integer), ad_client.domain (String), AD_SITE_TEMPLATE.foldername (String)]
     */
    private static Hashtable adClientDomainCache=new Hashtable();
    
    /**
     @param f true for debug mode, false for production mode
    */
    ...
    public static void unloadAdClientId(Integer adClientId){
    	for(Iterator it= adClientDomainCache.keySet().iterator();it.hasNext();){
    		Object key= it.next();
     ...
     * @param clientDomain, web_client.DOMAIN, should be case insensitive
    ...
    public static String getAdClientTemplateFolder(String webDomain){
    	Object[] dc=loadAdClient(webDomain);
    	if(dc==null) return null; // default to 001
    	return (String)dc[2];
    }
    /**
     * 
     * @param webDomain web_client.domain
     * @return null if not found, or  [ad_client.id (Integer), ad_client.domain (String)]
     */
    private static Object[] loadAdClient(String webDomain){
    	if (webDomain==null) return null;
    	...
    	try{
    		if(dc==null){
    	    	...
    	    	pstmt= conn.prepareStatement("select c.id, c.domain, t.FOLDERNAME from web_client w, ad_client c, AD_SITE_TEMPLATE t where w.domain=? and c.id=w.ad_client_id and t.id=w.AD_SITE_TEMPLATE_ID" );
    	    	pstmt.setString(1,d);
    		...
    		try{ if(rs!=null)rs.close();}catch(Throwable t2){}
    		try{ if(pstmt!=null)pstmt.close();}catch(Throwable t2){}
    		try{ if(conn!=null)conn.close();}catch(Throwable t2){}
    	}
		return dc;    	
    }
    ...
    private static int serverRunInSingleCompanyMode=-1; // 0 for false, 1 for true 
    /**
     * By default, NEA supports multiple companies mode, but when user authentication is based on 
     * email address, it will be single company mode
    ...
    public static boolean isServerRunInSingleCompanyMode(){
    	if(serverRunInSingleCompanyMode!=-1) return (serverRunInSingleCompanyMode==1);
    	try{
    	ServletContext ctx = getServletContext();
    	String companyId = (String)ctx.getAttribute(com.liferay.portal.util.WebKeys.COMPANY_ID);
    	com.liferay.portal.model.Company company =
    		com.liferay.portal.service.CompanyLocalServiceUtil.getCompany(companyId);

    	serverRunInSingleCompanyMode= (company.getAuthType().equals(
    			com.liferay.portal.model.impl.CompanyImpl.AUTH_TYPE_EA)? 1:0);
    	}catch(Throwable t){
    ...
    public static synchronized ServletContextManager getServletContextManager( ) {
        ...
        /*        if( context ==null){
                    //trying to get from JNDI tree
                        Context initial=null;
                        try{
                            initial= new InitialContext();
                            manager= (ServletContextManager)initial.lookup(JNDINames.CONTEXTMANAGER_WEB);
        ...
        try {
            ...
            e.printStackTrace();
            throw new NDSRuntimeException("Error getting ServletContextManager", e);
        }
    }
    ...
    /**
     * Check for whether has permission on querying specific table. We provide following rule on
     * converting table name to directory name, which is used for assigning permissions:
     *
    ...
    public static void checkTableQueryPermission(String tableName, HttpServletRequest request) throws NDSSecurityException {
        String directory;
        directory= TableManager.getInstance().getTable(tableName).getSecurityDirectory();

        WebUtils.checkDirectoryReadPermission(directory, request);
    }
    
    /**
     * @param permission can be one of Directory.READ or Directory.WRITE
     */
    private static void checkDirectoryPermission(String dirName, int permission,HttpServletRequest request ) throws NDSSecurityException {
        ...
        /*//marked for guest visit
        if( session ==null || session.isNew())
            throw new NDSSecurityException("必须登录系统才能访问受保护的对象"); */
     ...
     * @param Table if null, will return ObjectUIConfig.getDefaultTableUIConfig()
     * @return 
     */
    public static ObjectUIConfig getTableUIConfig(Table table){
    	if(table ==null) return ObjectUIConfig.getDefaultTableUIConfig();
    	...
    	ObjectUIConfig uiConfig= (ObjectUIConfig)pcManager.getPortletConfig(table.getUIConfigId(),nds.web.config.PortletConfig.TYPE_OBJECT_UI);
    	if(uiConfig==null) uiConfig=ObjectUIConfig.getDefaultTableUIConfig();
    	return uiConfig;
    }
    /**
     ...
     * @throws Exception
     */
    /*public static int getObjectPermission(String tableName, int objectId, int userId)throws Exception{
    	int perm=0;
    ...
    /**
     * 将字符串转化成JavaScript能够用document.write()方法输出的文本
     * 1. 将单引号(\')转化为双引号(\")
     * 2. 将回车转换为\\n
     * 3. 将换行转换为\\r
     * 异常输出时使用，如：/header.jsp
     * @param str
     * @return
     */
    public static String stringForJsOutput(String str){
        ...
        j = getBigNum(k,l) == -1?str.length():getBigNum(k,l);
        if(i<str.length())res += str.substring(i,j);

        return res;*/
        return str;
    }
    /**
     * 删除字符串中的WebKeys.MESSAGE值
     * 在/objext/sheet_item.jsp中使用
     * @param msg
     * @return
     * @deprecated
     */
    /*public static String removeValueholdMessage(String msg){
        int index = msg.indexOf(WebKeys.MESSAGE);
        String res = null;
        if(index != -1){
            if(msg.charAt(index - 1) == '&')
                res = msg.substring(0,index - 1);
            else
                res = msg.substring(0,index);
            int end = msg.indexOf("&", index);
            if(end != -1){
                res += msg.substring(end);
            }
        }else return msg;
        return res;
    }*/
     ...
     * be set in configuration file, named using nds.util.WebKeys.NDS_PROPERTIES_FILE
     * and the item is named "company.name"
     *
     * @return string of the company name, if "company.name" not found in
     * nds.util.WebKeys.NDS_PROPERTIES_FILE, then default company name "ACT" will be used.
     */
    public static String getCompanyInfo(){
        ...
        String name="ACT";
        try{
            name=new String(conf.getProperty("company.name","ACT").getBytes("8859_1"),"GB2312");
        }catch(Exception e){
            e.printStackTrace();
        }
        companyName=name;
     ...
     * Will use MessagesHolder to do internal wildcard replacement
    ...
    public static String getExceptionMessage(Throwable t, Locale locale){
    	...
    	if(s==null) return nds.util.MessagesHolder.getInstance().translateMessage("@unknown-exception@", locale);
    	//if(isSystemDebugMode()) return nds.util.MessagesHolder.getInstance().translateMessage(s, locale);
    	s= s.trim();
    	int p= s.indexOf("ORA-");
    ...
    private static String parseOracle001Error(String s, int fromIndex, Locale locale){
    	...
    	List al=nds.query.QueryEngine.getInstance().doQueryList("select table_name, column_name from USER_IND_COLUMNS where column_name<>'AD_CLIENT_ID' and index_name="+ 
    			QueryUtils.TO_STRING(indexName));
    	if(al.size()>0){
        	StringBuffer sb=new StringBuffer("@columns-that-violate-unique-constraint@:");
    		for(int i=0;i< al.size();i++){
    			...
    			if(col!=null) cname= col.getDescription(locale);
    			else cname= tname+"."+cname;
    			sb.append(cname);
    		}
    ...
    /**
     * If in debug mode, the sytem application dictionary can be modified, and page will 
     * show debug information for easy developing
    ...
    public static boolean isSystemDebugMode(){
    	if(systemMode==0){
	        ...
	        try{
	            ...
	            if("develope".equals(mode)) systemMode=1;
	            else systemMode=2;
	        }catch(Exception e){
	            e.printStackTrace();
	        }
    ...
    private static void checkLicense(){
        ...
        count=0;
        try{
        java.sql.ResultSet  rs= nds.query.QueryEngine.getInstance().doQuery("select value from appsetting where name='licd' and module='sys'");
        if ( rs.next() == true ){
            String d= rs.getString(1);
            if ( System.currentTimeMillis() -  ( new Long(d)).longValue() > 1000*60*60*24*14){
                System.out.print("\n\n\n\n########### NDS License expired ########## \n\n");
                System.exit(1);
            }
        }else {
    ...
    public static void loginSSOUser(User user, HttpServletRequest request,HttpServletResponse res )throws Exception{
    	com.liferay.portal.action.LoginAction.loginSSOUser(request, res, user.email);
    }    
    ...
    public static String getUserWelcomeString(HttpServletRequest request){
    	...
    	try{
    		...
    		if(scm!=null) userWeb= ((nds.control.web.UserWebImpl)scm.getActor(nds.util.WebKeys.USER));	
    	}catch(Throwable userWebException){
    	}
    	if(userWeb==null || userWeb.isGuest()){
    		return welcomeNewUser;
    	}else{
    		return "您好，" + userWeb.getUserDescription()+"，<a href='/c/portal/logout' class='reguser'>退出系统</a>";
    	}
    }
    /**
 	 * Parse reference column setting in wildcard filter
 	 * @param searchOnColumn the column which may contains wildcard filter
 	 * @param request,param
 	 * 		wfc_<columnid> - when column isFilteredByWildcard, values of those reference columns can be fetched here
 	 * @return null if not found
 	 */
 	public static Expression parseWildcardFilter(Column searchOnColumn, javax.servlet.http.HttpServletRequest request,UserWebImpl userWeb)throws Exception{
 		if(searchOnColumn==null || !searchOnColumn.isFilteredByWildcard()) return null;
 		...
 		for(int i=0;i<wfc_rcs.size();i++){
 			...
 			if(wfc_rcv!=null){
 				...
 				if(wfc_rc.getReferenceTable()!=null ){
 					...
 					/* 
 					 * the string will replace into wildcard filter directly, so when rc is numeric type
 				 	 * , return xxx, when string, return 'xxx', others not supported yet
 				 	 */ 					
 		...
 		Expression expr=new Expression(null, filter,
 				searchOnColumn.getDescription(locale)+ MessagesHolder.getInstance().getMessage(locale, "-have-special-filter"));
 		return expr;
 	}
 	/**
 	 * Parse reference column value in wildcard filter
 	 * @param value
 	 * @param rc reference column, which should has getReferenceTable()!=null
 	 * @return id of 
 	 * @throws Exception
 	 */
 	private static String parseReferenceColumnValue(String objectStr, Column col,UserWebImpl userWeb) throws Exception{
		...
		String refTablleDesc  = refTable.getDescription(Locale.CHINA) ;
	      ...
	      Column akColumn = refTable.getAlternateKey();
 	      ...
 	      StringBuffer sqlStr=new StringBuffer( "SELECT ID FROM "+(refTable.getRealTableName())+
 	      ...
 	      try{
 	        ...
 	        pstmt.setString(1,isUpperCase? objectStr.toUpperCase():objectStr );
            ...
            if(result.next() ){
 	          ...
 	          if(result!=null) try{ result.close();}catch(Exception e3){}
