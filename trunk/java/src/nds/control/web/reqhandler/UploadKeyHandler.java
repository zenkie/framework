...
public class UploadKeyHandler extends RequestHandlerSupport {
	...
	private static Logger logger = LoggerManager.getInstance().getLogger(
			UploadKeyHandler.class.getName());

	public UploadKeyHandler() {
	}

	public NDSEvent processRequest(HttpServletRequest request)
		...
		try {
			...
			factory.setSizeThreshold(1024 * 1024 * Tools.getInt(
					conf.getProperty("import.excel.maxsize", "1"), 1)); // 1MB
			factory.setRepository(new File(conf.getProperty("dir.tmp",
					"/portal/act.nea/tmp")));
			...
			upload.setSizeMax(1024 * 1024 * Tools.getInt(
					conf.getProperty("import.excel.maxsize", "1"), 1));
			...
			String absFileName = conf.getProperty("license", null);
			try {
				fileItem.write(new File(absFileName));
			} catch (java.io.FileNotFoundException fio) {
				throw new NDSEventException("@eception@:" + fio);
			}
