...
/**
 * ∆¿∑÷≈≈√˚
 * call C_PROJECT_ScoreCalc(projectno)
 */

public class AHYY_ScoreCalc extends Command {
	/**
	 * @param event
	 */
  public ValueHolder execute(DefaultWebEvent event) throws RemoteException, NDSException {
	...
	try{
		...
		int pid=Tools.getInt(engine.doQueryOne("select id from ad_process where PROCEDURENAME='C_PROJECT_SCORECALC'",conn),-1);
	  	if(pid==-1) 
	  		throw new NDSException("Internal Error: process 'C_PROJECT_SCORECALC' is not found");
	    ...
	    int piId=ProcessUtils.createAdProcessInstance(pid,ad_processqueue_name, recordno,usr,params,event.getData(),conn);
