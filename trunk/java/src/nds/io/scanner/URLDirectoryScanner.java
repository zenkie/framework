   ...
   implements DeploymentScanner
{
      ...
      throws MalformedURLException
   {
      ...
      if (compClassName != null)
      {
         ...
         {
            log.info("Unable to create instance of Comparator.  Ignoring.", e);
         }
      }
      // create a new filter
      ...
      if (filterClassName != null)
      {
         ...
         {
            log.info("Unable to create instance of Filter.  Ignoring.", e);
         }
      }
      ...
      synchronized (scanners)
      {
         ...
         for (int i = 0; i < list.getLength(); i++)
         {
            ...
            if (node.getNodeType() == Node.ELEMENT_NODE) {
               ...
               if (name == null)
               {
                  log.info("No name specified in URLDirectoryScanner config: " +
                  node + ".  Ignoring");
                  continue;
               }
               ...
               {
                  log.info("Invalid url in DeploymentScanner.  Ignoring.", e);
               }
            }
         }
      }
   ...
   public void setURLComparator(String comparatorClassName)
   {
      ...
      {
         log.info("Unable to create URLComparator.", e);
      }
   }
         ...
         if (lastModified != getLastModified())
         {
            if (exists())
            {
               // url needs deploy / redeploy
                  log.debug("Deploying Modified (or new) url: " + url);
                  deploy();
            } else
            {
   ...
   private class DirScanner extends Scanner
   {
      ...
      public void scan()
      {
         ...
         if (!dir.exists())
         {
            log.info("The director to scan does not exist: " + dir);
            return;
         }
         if (!dir.isDirectory())
         {
            log.info("The directory to scan is not a directory: " + dir);
            return;
         }
         ...
         while (true)
         {
            ...
            switch (comp.compare(file, scanner == null ? null : scanner.url))
            {
               case -1: // the file is new.  Create the scanner
                  log.debug("Found new deployable application in directory " +
                     dir + ": " + file);
                  try {
                     scanner = new Scanner(file.toURL());
                     deployed.add(deployedIndex, scanner);
                  } catch (MalformedURLException e) {
                     log.info("Unable to obtain URL for file: " + file, e);
                     fileIndex++;
                  }
               ...
               case 1: // the file has been removed.  A normal scan will remove it
                  log.debug("Deployed application removed from directory " +
                     dir + ": " + file);
                  scanner.scan();
                  if (!scanner.deployed)
                  {
