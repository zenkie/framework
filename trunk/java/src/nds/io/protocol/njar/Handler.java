...
import nds.io.protocol.Streams;
import nds.io.protocol.ThrowableHandler;
   ...
   extends URLStreamHandler
{
   ...
   private static final Logger log =LoggerManager.getInstance().getLogger(Handler.class.getName());
      ...
      if (embeddedURL.startsWith(PROTOCOL))
      {
         ...
         if (tempJar == null)
         {
            ...
            try {
               ...
               if (trace) log.debug("copied " + bytes + " bytes");
            }
            finally {
         ...
         if (trace) log.debug("file URL : " + t);
         
         t = "njar:" + t + NJAR_SEPARATOR + jarPath;
         if (trace) log.debug("Opening saved jar: " + t);

         URL u = new URL(t);
         if (trace) log.debug("Using URL: " + u);
         
         return u.openConnection();
      }
      else
      {
         if (trace) log.debug("Opening final nested jar: " + embeddedURL);
