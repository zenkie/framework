 ...
 * classname. The default JBoss package "nds.io.protocol" is searched
...
{
   /** Class logger. */
   private static final Logger log = LoggerManager.getInstance().getLogger(URLStreamHandlerFactory.class.getName());
   
   /** The package prefix where JBoss protocol handlers live. */
   public static final String PACKAGE_PREFIX = "nds.io.protocol";
   ...
   */
   ...
   /** The current packages prefixes determined from the java.protocol.handler.pkgs
    property + the nds.io.protocol default package.
    */
   ...
   public static void preload()
   {
      for (int i = 0; i < PROTOCOLS.length; i ++)
      {
         try
         {
            ...
            log.debug("Loaded protocol: " + PROTOCOLS[i]);
         }
         catch (Exception e)
         {
            log.error("Failed to load protocol: " + PROTOCOLS[i], e);
         }
      }
   }
      ...
      for(int p = 0; p < handlerPkgs.length; p ++)
      {
         try
         {
            ...
            if( type != null )
            {
               ...
               log.debug("Found protocol:"+protocol+" handler:"+handler);
            }
         }
